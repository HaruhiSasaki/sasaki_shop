...
public class MainController {
	...
	@Autowired // This means to get the bean called userRepository
	// Which is auto-generated by Spring, we will use it to handle the data
	private Maker_master_repository maker_master_repository;

	@PostMapping(path = "/addmaker") // Map ONLY POST Requests
	public @ResponseBody String addNewMaker_master(@RequestParam String makercd, @RequestParam String makername,
			@RequestParam String torokusya, @RequestParam String torokupg,
			@RequestParam Timestamp torokudate, @RequestParam String kosinsya, @RequestParam String kosinpg,
			@RequestParam Timestamp kosindate,@RequestParam Integer version) {
		// @ResponseBody means the returned String is the response, not a view name
		...
		m.setMakercd(makercd);
		m.setMakername(makername);
		m.setTorokusya(torokusya);
		m.setTorokupg(torokupg);
		m.setTorokudate(torokudate);
		m.setKosinsya(kosinsya);
		m.setKosinpg(kosinpg);
		m.setKosindate(kosindate);
		m.setVershion(version);

		maker_master_repository.save(m);
		return "Saved";
	}

	@GetMapping(path = "/allmaker")
	public @ResponseBody Iterable<Maker_master> getAllMaker_masters() {
		// This returns a JSON or XML with the producr_masters
		return maker_master_repository.findAll();
	}
